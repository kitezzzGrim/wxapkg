...
func Unpackall(c *cli.Context) error {
	in := c.String("in")
	out := c.String("out")
	//v := c.Bool("v")
	//format := c.Bool("format")
	v := true      // 将 -v 参数设置为默认值 true
	format := true // 将 --format 参数设置为默认值 true
	...
	err := Unpack(in, out, format, infof)
	if err != nil {
		if errors.Is(err, ErrInvalidWXAPkg) {
			err_.Printf("[-] '%s' %s\n", in, err.Error())
		}
